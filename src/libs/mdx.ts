...
import { TOKEN_CLASSNAME_MAP } from '@/constants'
import { TokenClassNameMap } from '@/types/general'
import { dateSortDesc } from '@/utils/date'

export async function getFileBySlug(type: string, slug: string): Promise<MdxFileData> {
  let root = process.cwd()
  let mdxPath = path.join(root, 'src', 'data', type, `${slug}.mdx`)
  let mdPath = path.join(root, 'src', 'data', type, `${slug}.md`)
        ...
        () => {
          return (tree) => {
            visit(tree, 'element', (node: UnistNodeType) => {
              ...
              const typeMap = type as keyof TokenClassNameMap
              if (token === 'token' && node?.properties?.className) {
                node.properties.className = [TOKEN_CLASSNAME_MAP[typeMap]]
              }
            })
          }
        },
...
export function getAllFilesFrontMatter(folder: string) {
  let root = process.cwd()
  let prefixPaths = path.join(root, 'src', 'data', folder)
