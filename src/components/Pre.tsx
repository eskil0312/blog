...
export function Pre({ children }: { children?: React.ReactNode }) {
  let textInput = useRef<HTMLDivElement | null>(null)
  ...
  const onCopy = () => {
    ...
    const current = textInput?.current
    const innerTextCurr = current?.innerText
    if(!innerTextCurr){
        return
    }
    const innerText = innerTextCurr.replace(/(\$ |copy)/g, ' ')
